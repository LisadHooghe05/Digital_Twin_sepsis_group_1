...
def get_vasopressor_matrix_12h_1():
    """
    Builds a wide matrix of vasopressor doses per subject within 12h before AKI onset.
    Rows: subject_id
    Columns: vasopressor types, values: summed dose amounts
    ...
    aki_stage_file = "AKI_stages_subject_AKI.csv"
    ...
    # Merge vaso doses with vasopressor info to get subject_id and type
    vaso_merged = pd.merge(
        vaso_df,
        vaso_info_df[['subject_id','stay_id','vasopressor_type']],
        on='stay_id',
        how='left'
    )
    ...
    # Filter rows within 12h before AKI onset
    vaso_merged['hours_before_AKI'] = (vaso_merged['AKI_time'] - vaso_merged['endtime']).dt.total_seconds() / 3600
    vaso_12h = vaso_merged[(vaso_merged['hours_before_AKI'] >= 0) & (vaso_merged['hours_before_AKI'] <= 12)].copy()

    # Drop rows with missing values in essential columns
    vaso_12h = vaso_12h.dropna(subset=['vasopressor_type','amount','AKI_stage'])
...
#vaso_matrix = get_vasopressor_matrix_12h()
#print(vaso_matrix)
